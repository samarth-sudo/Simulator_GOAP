...
{
    public class GOAPDriver_DemoHelicopter : GOAPDriver
    {
        ...
        {
            return pilot.helicopter.rb.velocity;
        }

        protected override void MoveVelocity(Vector3 velocity)
        {
            pilot.SetTargetPosition(tf.position, velocity);
            if (pilot.helicopter.rb.velocity.magnitude > 10)
            {
                pilot.SetAimDir(pilot.helicopter.rb.velocity);
            }
        }
        ...
        protected override void Panic()
        {
            ...
            if (pilot.helicopter.rb.velocity.magnitude > crawlSpeed)
            {
        ...
        {
            return new GOAPWorldState_SimpleAircraft(tf, pilot.helicopter.rb);
        }
    }
}